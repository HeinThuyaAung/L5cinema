...
if (isset($_REQUEST['id'])) 
{
	...
	date_default_timezone_set("Asia/Rangoon"); 
	$tdate=date('d-M-Y');
	$update="UPDATE Booking SET PaymentStatus='Paid',PaymentDate='$tdate' WHERE BookingID='$ID'";
	$ret=mysql_query($update);
	if ($ret>0) 
	{
		echo '<script>window.alert("Paid For Booking!");
		window.location="Booking_Payment.php";</script>';
	}
	else if($ret<=0)
	{
		echo "<script>window.alert('Invalid')</script>";
		echo "<script>window.location='Booking_Payment.php'</script>";
	}
	else
	{
		echo mysql_error();
...
<head>
        ...
        <!-- Basic -->
        <title>Cinema | PaymentStatus</title>
...
<script>
function myFunction() {
    var npassword = prompt("Please enter Paid Date","<?php date_default_timezone_set("Asia/Rangoon"); echo date('d-M-Y'); ?>");
    if (npassword = null) {
...
<section id="contact" class="contact">
        <div class="container">
			...
			<div class="row">
                <div class="col-lg-12">
                        <div class="row">
<form action="Booking_Payment.php" method="POST" enctype="multipart/form-data" name="user" id="contactForm" novalidate>
<input type="hidden" name="txtcid" id="demo" value="<?php if(isset($CID)) {echo $CID;} ?>">
                        	<table>
...
<?php 
	...
	{ 
		...
		echo "<td><a href='Booking_Payment.php?id=".$data['BookingID']."' onclick='myFunction()'>Paid</a></td>";
		echo "</tr>";
	}
